...
namespace PuzzleMash._8._1
{
    ...
    public sealed partial class MainPage : Page
    {
...
#if WIN10
        public async string[] DirectoryGetFiles(string directory)
        {
...
#if WIN8_1
        public IReadOnlyList<StorageFile> DirectoryGetFiles(string directory)
        {
            var root = Windows.ApplicationModel.Package.Current.InstalledLocation.Path;
            var directoryTask = StorageFolder.GetFolderFromPathAsync(root + "\\Res\\Image\\" + directory + "\\").AsTask();
            directoryTask.Wait();
            var filesInFolder = directoryTask.Result.GetFilesAsync().AsTask();
            filesInFolder.Wait();
            return filesInFolder.Result;
        }
#endif
        ...
        {
            var task = StorageFile.GetFileFromApplicationUriAsync(new Uri(fileName.Replace("ms-appx:/Res", "ms-appx:///Res"), UriKind.RelativeOrAbsolute)).AsTask();
            task.Wait();
            return task.Result;
        }
